...
export function ControlPanel(props: Props) {
  ...
  return (
    ...
    >
      <FormControl size='small' sx={isSidebar ? undefined : { minWidth: { xs: '100%', md: 290 } }}>
        <InputLabel id='chart-type-label'>
          <FieldInfoLabel
            label='Chart type'
            description='Choose visual mode. Each mode has its own settings below.'
          />
        </InputLabel>
        <Select
      ...
      {props.chartType === 'xy' ? (
        <FormControl size='small' sx={isSidebar ? undefined : { minWidth: { xs: '100%', md: 230 } }}>
          <InputLabel id='metric-label'>
            <FieldInfoLabel
              label='Y metric (XY)'
              description='Steps to 1: number of transitions to reach 1. Peak value: maximum value seen in the trajectory.'
            />
          </InputLabel>
          <Select
